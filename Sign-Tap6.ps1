...
Function Show-Usage {
    Write-Host "Usage: Sign-Tap6.ps1 -SourceDir <sourcedir> [-Force] [-VerifyOnly] [-Append]"
    Write-Host
    Write-Host "Example 1: Sign for OpenVPN (OSS)"
    Write-Host "    Sign-Tap6.ps1 -SourceDir tap6 -Force"
    Write-Host
    Write-Host "Example 2: Sign for OpenVPN Connect (Access Server)"
    Write-Host "    Sign-Tap6.ps1 -SourceDir tapoas6 -Force"
    Write-Host
    Write-Host "Example 3: Append a signature to a signed driver"
    Write-Host "    Sign-Tap6.ps1 -SourceDir tap6 -Append"
    Write-Host
}
...
foreach ($file in $cat_x86,$cat_x64,$devcon_x86,$devcon_x64) {
    ...
    if ( ($not_signed) -or ($Append) ) {
        ...
        & $signtool timestamp /tr $timestamp /td $digest /tp $idx $file
    } else {
        Write-Host "${file} is signed already, not signing it"
    }
}

# Create a tarball with the signed driver. This is disabled for now, because
# running tar bundled with Git Bash fails in interesting ways when tar is
# running inside a Powershell session.
#if ( ! (Test-Path $tarball)) {
#    & ${tar} -zvcf ${sourcedir_basename}-signed.tar.gz ${sourcedir_basename}
#} else {
#    Write-Host "Signed tarball present, not generating it."
#}
